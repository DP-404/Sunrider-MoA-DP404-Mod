...
define kay = Character('Kayto',     what_outlines=[(5, "#000000", 1, 1),(2, "#575757", 0, 0) ], who_outlines=[(5, "#000000", 1, 1),(3, "#575757", 0, 0)] )
define asa = Character('Asaga',     what_outlines=[(5, "#000000", 1, 1),(2, "#be4c58", 0, 0) ], who_outlines=[(5, "#000000", 1, 1),(3, "#be4c58", 0, 0)] )
define chi = Character('Chigara',   what_outlines=[(5, "#000000", 1, 1),(2, "#615a9b", 0, 0) ], who_outlines=[(5, "#000000", 1, 1),(3, "#615a9b", 0, 0)] )
define ava = Character('Ava',       what_outlines=[(5, "#000000", 1, 1),(2, "#4a2a1a", 0, 0) ], who_outlines=[(5, "#000000", 1, 1),(3, "#4A2A1A", 0, 0)] )
define sol = Character('Sola',      what_outlines=[(5, "#000000", 1, 1),(2, "#75798a", 0, 0) ], who_outlines=[(5, "#000000", 1, 1),(3, "#75798A", 0, 0)] )
define mar = Character('Maray',     what_outlines=[(5, "#000000", 1, 1),(2, "#979797", 0, 0) ], who_outlines=[(5, "#000000", 1, 1),(3, "#979797", 0, 0)] )
...
label after_mission2:
    ...
    if blackjack == None: # it shouldn't be possible to kill Havoc on the first turn, but if the player did... just in case
            $ blackjack_weapons = [BlackjackMelee(),BlackjackLaser(),BlackjackAssault(),BlackjackMissile(),BlackjackPulse()]
            $ blackjack = create_ship(BlackJack(),(6,3),blackjack_weapons)
...
label fornowsunriderrepairs:
    ...
    ava "Anyways, what are you waiting for? The quicker we get to Tydaria, the less I need to worry about a fleet of pirate ships warping in around us."
    ava "Just use the star map and order the Sunrider to the workshop. I've already put in its coordinates on the computer, so you should be able to find it without any problems."
...
label completeourtestsandresupply:
    ...
    kay "{i}We're about to set sail again. I'm still not sure what to do about our two guests. Even though Ava still doesn't trust them, they've been an invaluable help. I think they'd do well on board the Sunrider.{/i}"
    kay "... ... ..."
...
label attackonpiratesnest:
    ...
    $ chigara_liberty_explanation = False
...
label mission3:
    ...
    if check1 == False:
        ...
        $ chigara_liberty_explanation = True
...
label piratebaseattack:
    ...
    scene asteroid back3 with battlewipe
...
label mission5:
    ...
    if check2 == False:
        ...
        if chigara_liberty_explanation == False:
    ...
    if battle_check1 == False and BM.turn_count == 5:
        ...
        $ PlayerTurnMusic = None #"Music/Sora_no_Senritsu.ogg"
        $ EnemyTurnMusic = None #"Music/Sora_no_Senritsu.ogg"
        play music "music/Sora_no_Senritsu.ogg" fadeout 2.0
...
label chap2_start:
    ...
    kay "{i}It's been a two weeks since we took down Cosette's base in the asteroids.  Since then, we've been chasing down minor criminals and undertaking hit and run attacks on PACT supply lines.{/i}"
    kay "{i}Despite our efforts, PACT's invasion of the Neutral Rim has continued unabated.{/i}"
    kay "{i}Six more governments have voluntarily joined PACT and two more have been conquered.  PACT's war machine has been indiscriminate, slaughtering both civilians and combatants alike.{/i}"
    kay "{i}In the mere month since Cera, millions more innocents have been murdered.{/i}"
    kay "{i}Unless something is done, the day when PACT rules the entire Neutral Rim appears to be fast approaching.{/i}"
...
label proceed_rescuediplomats:
    ...
    kay "Begin captain's log. {i}We've made contact with the diplomats on Versta and are going to attempt an escape.{/i}"
    kay "{i}I should be focused on getting them out of here safe, but I can't get what the mercenary said out of my mind.{/i}"
    kay "{i}Some part of my head thinks that she has some ulterior motive and she's tricking us all. Maybe she's a PACT spy and making us wander into a trap.{/i}"
    kay "{i}Maybe the diplomats know something that PACT doesn't and she's just fooling us into silencing them.{/i}"
    kay "{i}Or maybe she's really telling the truth.{/i}"
...
label cmd_savethechildren:
    ...
    if BM.cmd >= cmd_versta:
        menu:
            "...Maintain course. Prepare to break through that PACT squadron.":
                jump maintainpreparethroughpact
            "COMMAND DECISION: Cancel warp. We're going to escort the Agamemnon out of here! |[cmd_versta] CMD/[BM.cmd] Available|":
...
label maintainpreparethroughpact:
    ...
    scene bg bridgered:
        ...
        ease 0.025 xalign 0.5
        repeat 2
    show asaga uniform handsonhips mad:
        xpos 0.2
        ease 0.025 xpos 0.19
        ease 0.05 xpos 0.21
        ease 0.025 xpos 0.2
        repeat 2
    show ava uniform armscrossed looklefttalk:
        xalign 0.5
        ease 0.025 xalign 0.45
        ease 0.05 xalign 0.55
        ease 0.025 xalign 0.5
...
label after_mission11:
    ...
    kay "Begin captain's log. {i}We've arrived at the Alliance world of Far Port. News of Ryuvia's fall has spread throughout Alliance space like wildfire.{/i}"
    kay "{i}Now, there can be no doubt about Arcadius' ambitions. With PACT now at its doorsteps, the Solar Congress has convened for an emergency session. The only possible outcome is a declaration of war.{/i}"
    kay "{i}The Sunrider is now caught right in the middle of the biggest intergalactic war of our time.{/i}"
    kay "{i}I'm not sure just where we stand here..."
...
label gutshouldntalliance:
    ...
    kay "{i}The Alliance is mired in politics. Whatever is good for the Alliance may not be in the best interests of my ship.{/i}"
    kay "{i}For now, it'll be better to wait and see just what the Alliance wants out of this war.{/i}"

    jump asagaafterrescue

label thatassistalliance:

    kay "{i}The Alliance is our best hope of defeating PACT. We have to accept the fact that we're just one ship against an entire empire. We need powerful allies if we're going to survive.{/i}"
...
label needhelpyou:
    ...
    if CENSOR == True:
        scene cg_asagashower1_censored with dissolvelong
    if CENSOR == False:
    ...
    if CENSOR == True:
        scene cg_asagashower2_censored with dissolvelong
    if CENSOR == False:
        scene cg_asagashower2 with dissolvelong
...
label beachepisode(reset_lowest_difficulty = False):
    ...
    ava "Begin First Commander's log. {i}It has been months since we've been marooned on this godforsaken place.{/i}"
    ava "{i}Day by day, the crew's discipline begins to decay. I feel the chain of command dissolving with each day.{/i}"
    ava "{i}I can see it in the eyes of the crew. The chances of a rescue are remote. Some of the rowdiest of our crew have even suggested starting a... settlement! Here! So far away from the raiment of civilization!{/i}"
    ava "{i}I fear it is only a matter of time until we lose all control over the crew and we have a mutiny on our hands...{/i}"

    show ava uniform fistup angry behind beach_over with dissolve

    ava "{i}Until then... I, First Officer Ava Crescentia, will do my best to maintain order!{/i}"
    ava "{i}Even if...{/i}"
...
label checkasaga:
    ...
    if chat_asaga == True:
        kay "I've already talked with Asaga. I should talk with someone else."
        jump directchat

label checkchigara:

    if chat_chigara == False:
        $ chat_chigara = True
        jump beachchigara

    if chat_chigara == True:
        kay "I've already talked with Chigara. I should talk with someone else."
        jump directchat
...
label checkava:
    ...
    if chat_ava == True:
        kay "I've already talked with Ava. I should talk with someone else."
        jump directchat

label checkicarikryska:

    if chat_icarikryska == False:
        $ chat_icarikryska = True
        jump beachicarikryska

    if chat_icarikryska == True:
        kay "I've already talked with Icari and Kryska. I should talk with someone else."
        jump directchat
...
label checksola:
    ...
    if chat_sola == True:
        kay "I've already talked with Sola. I should talk with someone else."
        jump directchat

label checkclaude:

    if chat_claude == False:
        $ chat_claude = True
        jump beachclaude

    if chat_claude == True:
        kay "I've already talked with Claude. I should talk with someone else."
        jump directchat
...
label clearoutpirates:
    ...
    scene asteroid back2 with battlewipe
...
label after_mission14:
    ...
    $ remove_order('RETIRADA') #using this function is safer than using del
...
label after_mission16:
    ...
    scene cg_ongess_carrier_back with battlewipe
...
label bogiespiratefind:
    ...
    scene bg bridgered:
        ...
        ease 0.025 xalign 0.5
        repeat 2
    show ava uniform armscrossed angry:
        xpos 0.5
        ease 0.025 xpos 0.5
        ease 0.05 xpos 0.51
        ease 0.025 xpos 0.49
...
label goodbyemaray:
    ...
    play sound1 "Sound/explosion4.ogg" #sound1
    ...
    play sound1 "Sound/explosion4.ogg" #explosion5
...
label satalonestaring:
    ...
    scene bg bridgered:
        ...
        ease 0.025 xalign 0.5
        repeat 2
    show ava uniform neutral angry:
        xpos 0.5
        ease 0.025 xpos 0.5
        ease 0.05 xpos 0.51
        ease 0.025 xpos 0.49
    ...
    scene paradox back with battlewipe
    ...
    if captain_moralist > captain_prince:
        ...
        if wishall == True:
            menu:
                ...
                "CMD DECISION: Get down there and get the Vanguard online! ([cmd_helion] CMD/[BM.cmd] Available)":
    ...
    if captain_prince > captain_moralist:
        ...
        if wishall == True:
            ...
            menu:
                ...
                "CMD DECISION: Are you crazy, commander!? It's too dangerous! ([cmd_helion] CMD/[BM.cmd] Available)":
...
label getvanguardthe:
    ...
    play sound "sound/legion_maincannon_charge.ogg"
    scene cg_finalstand2 with dissolvelong
    ...
    pause 1
    scene cg_finalstand3 with laserwipe
...
label aftercredits7:
    ...
    "All that and more, in Sunrider: Liberation Day!" # The Rebirth of the Holy Empire
...
label maskofarcadiuschoice:
    ...
    menu:
        "Play Sunrider First Arrival":
            jump start
        "Continue playing Sunrider Mask of Arcadius":
...
label rb_7:
    ...
    "When you found the ship in danger..."
...
label rb_10:
    ...
    "Before the operation to rescue Asaga..."
...
label rb_16:
    ...
    show vanguard1 with dissolvelong:
        xalign 0.65 yalign 0.45 zoom 0.75
