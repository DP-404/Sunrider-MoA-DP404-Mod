            ...
            for cover in BM.covers:
                if cover.location == target.location:
                    if renpy.random.randint(1,100) <= cover.cover_chance:
                        show_message(_('the shot was blocked by an asteroid!'))
                        renpy.pause(1.0)
                        total_damage = 0
            ...
            for ship in player_ships:
                for key in ship.modifiers:
                    ...
                    if mod_power != 0:
                        if duration == 1:
                            if mod_power < 0:
                                message = __("{0} recovered from curse to its {1}").format(__(ship.name), __(key).replace('_', ' '))
                                BM.battle_log_insert(['support', 'debuff'], message)
                                show_message(message)
                            else:
                                if not order_expired:
                                    message = __("{0} lost buff to its {1}").format(__(ship.name), __(key).replace('_', ' '))
